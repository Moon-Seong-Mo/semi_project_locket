...
ALTER TABLE WISHLIST ADD CONSTRAINT W_WNUM_FK FOREIGN KEY(WISH_NUM) REFERENCES PRODUCT ON DELETE CASCADE;
...
ALTER TABLE PRODUCT_COMMENT ADD CONSTRAINT PC_ONUM_FK FOREIGN KEY(O_NUM) REFERENCES PRODUCT ON DELETE CASCADE;
...
AS
SELECT ROWNUM RNUM, DESCPRODUCT.*
FROM (SELECT PRODUCT_NUM, MEMBER_ID, MEMBERNAME, CATEGORY, TITLE, PRICE, CONDITION, DELIVERY, EXPLAIN, CREATE_DATE, PRODUCT_COUNT
      FROM PRODUCT
           JOIN MEMBER USING(MEMBER_ID)
      WHERE STATUS = 'Y'
      ORDER BY PRODUCT_NUM DESC) DESCPRODUCT;
